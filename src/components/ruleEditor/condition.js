        ...
        return (
            <div key={new Date().getTime() + 'level' + level} className={cls(
                ...
                </Select>
                {
                    ...
                    <Select getPopupContainer={() =>document.getElementById("content")} onChange={changeConditionContent.bind(_this, node, index, 'value')} className={styles.value} style={{ width: 150 }} value={value} placeholder={window.__alert_appLocaleData.messages['ruleEditor.phTag']}>
                        {
                            valueList[_key] &&
