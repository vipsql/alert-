    ...
    module.exports = require('../../echarts').extendChartView({
        ...
        _onZoom: function (scale, mouseX, mouseY) {
            ...
            if (this._state !== 'animating') {
                ...
                var rootLayout = root.getLayout();
                if(!this.initLayoutWidth) this.initLayoutWidth = rootLayout.width;
                
                if (!rootLayout) {
                    return;
                }
                // 实现只能放大但不能缩小的效果
                if(rootLayout.width * scale < this.initLayoutWidth){
                    return
                }
            ...
            function onClick(e) {
                ...
                if (node.getLayout().isLeafRoot) {
                    ...
                    if (nodeClick === 'zoomToNode') {
                        ...
                        // add hash redirect
                    }else if(nodeClick === 'hash'){
                      var itemModel = node.hostTree.data.getItemModel(node.dataIndex);
                      var maxSeverity = itemModel.get('maxSeverity', true);
                      var severityName = maxSeverity == 0 
                                            ? '恢复' : maxSeverity == 10 
                                                ? '提醒' : maxSeverity == 20 
                                                    ? '警告,提醒' : maxSeverity == 30 
                                                        ? '次要,警告,提醒' : maxSeverity == 40 
                                                            ? '主要,次要,警告,提醒' : maxSeverity == 50
                                                                ? '紧急,主要,次要,警告,提醒' : ''
                   
                      var hash = itemModel.get('path', true);
                      var keyValue = itemModel.get('name', true);
                      var pathArr = hash.split('/');
                      var alertListPath = {};
                      if ( pathArr !== undefined && pathArr[0] !== undefined) {
                          var temp = pathArr[0];
                          alertListPath[temp] = keyValue;
                      }
                      alertListPath.severity = severityName;
                      localStorage.setItem('alertListPath', JSON.stringify(alertListPath));
                      window.location.hash = "#/alertManage/" + hash
                    }
